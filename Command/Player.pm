...
sub cmd_player
{
    ...
    if(index($search, "#") != -1) { 
        ...
        if($racial =~ /^$/) {
            ## just player no race
            $sth = $dbh->prepare("SELECT * FROM `everyone` WHERE `name` REGEXP '^".$search.".*' ORDER BY `mmr` DESC");
        }
        else {
            $search =~ s/$racial//g;
            $search =~ s/ //g;
            if($racial =~ /^(terran|zerg|protoss|random)$/i) {
                $sth = $dbh->prepare("SELECT * FROM `everyone` WHERE `name` REGEXP '^".$search.".*' AND `race` = '".$racial."' ORDER BY `mmr` DESC");
            }
            elsif($racial =~ /^(kr|na|us|eu|NA|KR|US|EU)$/i) {
                if($racial =~ /^na$/i) {
                    $racial = "us";
                }
                $sth = $dbh->prepare("SELECT * FROM `everyone` WHERE `name` REGEXP '^".$search.".*' AND `server` = '".$racial."' ORDER BY `mmr` DESC");
            }
            else {
                MakeDiscordPostJson("/webhooks/$WH_ID/$WH_Key", '{"content" : "please use a race or server: ~player shortland#952 **NA**"}', "1", "");
                return;
            }
        }
        ...
        while (my $row = $sth->fetchrow_hashref()) {
            ...
            $dbh->do('SET NAMES utf8mb4') or die($dbh->errstr);
            $dbh->{mysql_enable_utf8mb4} = 1;
            my $sth = $dbh->prepare("SELECT * FROM `everyone_social` WHERE `battle_tag` = '" . $BT . "' or `discord_tag` = '" . $BT . "'");
            $sth->execute();
            my $discord_link = $sth->fetchrow_hashref();
            if (defined $discord_link) {
            	$discord_link = $discord_link->{discord_tag};
            }
            else {
            	$discord_link = "";
            }

            my $if_exists_discord = "";
            if (defined $discord_link) {
                $if_exists_discord = '{"name": "Discord:", "value": "*' . $discord_link . '*", "inline": 0},';
            }
            
            $text = '{"content": "", "embeds": [{"footer": {"text": "Last Server Update: '.(scalar localtime $lastupdate).'"}, "author": {"icon_url": "http://138.197.50.244/images/'.uc($row->{race}).'.png", "name": "<'.$row->{clan_tag}.'> '.$row->{name}.'", "url": "http://'.$row->{server}.'.battle.net/sc2/en'.$row->{path}.'/"}, "thumbnail": {"url": "http://138.197.50.244/images/'.uc($row->{league}).'.png", "height": 60, "width": 60}, "type": "rich", "color": 4343284, "fields": [{"name": "Profile:", "value": "*http://'.$row->{server}.'.battle.net/sc2/en'.$row->{path}.'/*", "inline": 0},' . $if_exists_discord . ' {"name": "RankedFTW:", "value": "*http://www.rankedftw.com/search/?name=http://'.$row->{server}.'.battle.net/sc2/en'.$row->{path}.'/*", "inline": 0}, {"name": "Tier:", "value": "*'.$row->{tier}.'*", "inline": 1}, {"name": "MMR:", "value": "*'.$row->{mmr}.'*", "inline": 1}, {"name": "Wins:", "value": "*'.$row->{wins}.'*", "inline": 1}, {"name": "Losses:", "value": "*'.$row->{losses}.'*", "inline": 1}, {"name": "Longest Win Streak:", "value": "*'.$row->{longest_win_streak}.'*", "inline": 1}, {"name": "Current Win Streak:", "value": "*'.$row->{current_win_streak}.'*", "inline": 1}, {"name": "Last 1v1:", "value": "*'. (scalar localtime $row->{last_played_time_stamp}) .'*", "inline": 1}, {"name": "BattleNet Tag:", "value": "*'.$BT.'*", "inline": 1}]}]}';
